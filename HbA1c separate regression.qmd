...
```{r}
ggplot(diabetes_df) + geom_point(aes(y=HbA1c, x=female))
```

```{r}
ggplot(diabetes_df) + geom_density_2d_filled(aes(y=HbA1c, x=female))
```

```{r}
ggplot(diabetes_df) +
    geom_density_2d_filled(aes(y=HbA1c, x=female)) +
    geom_line(aes(x=female, y=reg_10$fitted.values), color="red", linewidth=2)
```
